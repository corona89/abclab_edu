...
Knowledge API 의 Base URL 은 시스템에서 제공됩니다. Knowledge API 페이지에서 확인할 수 있습니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| data | array[object] | 지식 목록 (각 항목은 지식 생성 Response 와 동일한 필드) |
...
**Path Parameter**: `dataset_id` (string) — 지식 아이디 (지식 페이지 URL 에서 확인 가능)
...
지식 생성 Response 와 동일한 형식 (업데이트된 retrieval_model_dict 포함)
...
| data.process_rule | object | 임베딩 방식 설정 (문서 생성 by Text 와 동일) |
...
문서 생성 (by Text) Response 와 동일한 형식
...
**Path Parameters**:
- `dataset_id` (string) — 지식 아이디
- `document_id` (string) — 문서 아이디 (문서 상세 페이지 URL 에서 확인 가능)

| 파라미터 | 타입 | 설명 |
|---------|------|------|
| name | string | 문서 이름 (수정이 필요할 경우) |
| text | string | 문서 컨텐츠 (수정이 필요할 경우) |
| process_rule | object | 임베딩 방식 설정 (문서 생성 by Text 와 동일) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| data | array[object] | 문서 목록 (문서 생성 Response 의 document 필드와 동일) |
| has_more | bool | 더 있는지 여부 |
| limit | int | 페이지당 조회 개수 |
| total | int | 총 개수 |
| page | int | 페이지 번호 |
...
청크 추가 Response 와 동일 + `total` (int) 필드 추가
